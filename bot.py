...
def pattern(sym):
    ...
    result=20<=h[-2]<=29 and 20<=h[-1]<=29
    if result:print(f'PATTERN on {sym}: {h[-2]}, {h[-1]}')
    return result
    ...
    while running:
        try:
            async with websockets.connect('wss://ws.binaryws.com/websockets/v3?app_id=1089') as ws:
                ...
                # Load recent accumulator history from statement
                await ws.send(json.dumps({"statement":1,"description":1,"limit":50,"offset":0,"action_type":"sell"}))
                for _ in range(5):
                    raw=await asyncio.wait_for(ws.recv(),timeout=10)
                    msg=json.loads(raw)
                    if msg.get('msg_type')=='statement':
                        txns=msg.get('statement',{}).get('transactions',[])
                        for t in reversed(txns):
                            lc=t.get('longcode','').lower()
                            if 'accumulator' in lc:
                                dur=t.get('duration',0)
                                for sym in SYMBOLS:
                                    if sym.lower().replace('_','') in lc or NAMES[sym].lower().replace(' ','') in lc:
                                        if isinstance(dur,int) and 0<dur<500:
                                            tick_history[sym].append(dur)
                                        break
                        break

                hist_summary='\n'.join([f'{NAMES[s]}: {tick_history[s][-3:] if tick_history[s] else "no history yet"}' for s in SYMBOLS])
                await tg(
                    f'🤖 <b>Bot Started</b>\n'
                    f'Balance: <b>${bal:.2f}</b>\n'
                    f'Watching Vol 25, 75, 100\n'
                    f'Stake: $5 | Growth: 5% | TP: $2.50\n'
                    f'Daily target: +$30 | Stop: -$10\n\n'
                    f'📊 <b>Tick history loaded:</b>\n{hist_summary}'
                )

                # Check pattern immediately on startup
                for sym in SYMBOLS:
                    if not in_trade and pattern(sym):
                        await tg(f'🎯 <b>Pattern on startup!</b>\n{NAMES[sym]}: {tick_history[sym][-2]}, {tick_history[sym][-1]}\nEntering trade!')
                ...
                async for raw in ws:
                    ...
                    if mt=='proposal_open_contract':
                        ...
                        profit=float(c.get('profit',0));ticks=c.get('tick_count',0) or 0
                        status=c.get('status')
                        ...
                        if status in('sold','expired') and cid==contract_id:
                            ...
                            if sym and ticks>0:
                                ...
                                if len(tick_history[sym])>50:tick_history[sym].pop(0)
                            e='✅' if profit>=0 else '❌'
                            await tg(f'{e} <b>Trade Closed</b>\nSymbol: {NAMES.get(sym,sym)}\nTicks: {ticks}\nP&L: <b>${profit:.2f}</b>\nDaily P&L: <b>${daily_pnl:.2f}</b>')
                            ...
                            if not in_trade and pattern(sym):
                                await tg(f'🎯 <b>Pattern!</b> {NAMES.get(sym,sym)}\nLast 2 ticks: <b>{tick_history[sym][-2]}</b>, <b>{tick_history[sym][-1]}</b>\nEntering trade!')
                                await ws.send(json.dumps({'buy':1,'price':STAKE,'parameters':{'contract_type':'ACCU','symbol':sym,'currency':'USD','growth_rate':GROWTH_RATE,'basis':'stake','amount':STAKE}}))
